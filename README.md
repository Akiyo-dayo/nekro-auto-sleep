...
- **动态人格化问候**：早安与晚安招呼告别死板的固定模板，通过系统级 Prompt 指令引导 LLM 结合当前人设风格、聊天上下文以及昨夜睡眠/梦境/打卡数据动态生成自然生动的回复
...
| 叫醒确认窗口 | `180` 秒 | 提问后的等待确认窗口时间 |
| 唤醒确认关键词 | `要,叫醒,醒来,起床,是,是的,确认` | 确认窗口期内直接触发唤醒的关键词（逗号分隔；亦可通过提及/@Bot确认） |
...
| 动态生成招呼与问候 | `true` | 为 true 时根据人设与上下文经由 LLM 动态生成问候，失败或关闭时回退固定话术 |
...
用户有效呼叫 → 【Bot已经睡了 要叫醒Bot吗？】(不经 LLM)
       ↓ 3 分钟内发送确认词（如"要"、"叫醒"）或再次提及/回复 Bot
       → 切换为 AWAKE_EARLY，本轮正常进 LLM
       → 注入"你刚刚被用户提前叫醒了"（不写历史）
       ↓ （若发送"不要"/"算了"则取消唤醒；群聊闲聊无关消息继续静默拦截）
       ↓ 10 分钟无新互动 → 静默睡回
