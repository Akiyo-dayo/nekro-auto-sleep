...
def compute_fragmentation(
    ...
    Segments still open (``close_at is None``) count up to ``open_end`` when
    provided: settlement computes fragmentation BEFORE closing the final
    segment, and skipping open segments made every night look fully
    fragmented (draining a further fixed chunk from the quality score).
    """
    if actual_sleep <= 0:
        return 1.0
    sum_sq = 0.0
    for seg in segments:
        ...
        if seg_end is None:
            continue
        dur = (seg_end - seg.open_at).total_seconds()
        if dur > 0:
            sum_sq += dur * dur
...
def compute_quality(
    ...
    fragmentation = _clamp(
        compute_fragmentation(
            cycle.sleep_segments, actual_sleep_seconds, open_end=cycle.planned_wake_at
        ),
        0.0,
        1.0,
    )
