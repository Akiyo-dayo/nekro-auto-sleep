...
### 调整：梦境题材改为"自由发挥 + 可配置负面约束"

- **变更**：移除 v1.2.3 的梦境题材池（`DREAM_TOPICS` / `dream_topic_hint`），
  恢复 AI 结合记忆、人设与聊天上下文的自由发挥；防坍缩改由**负面约束**承担。
- **新增配置 `DREAM_TOPIC_CONSTRAINTS`**（默认 `不得与昨晚的梦境题材重复`）：
  多条约束用换行/分号分隔，注入晨间 prompt 的「梦境负面约束（必须遵守）」；
  留空则不加约束。prompt 同时要求参考聊天记录里昨晚的梦、避免每天落入同一类题材。
- 其余 v1.2.3 修复不变。
